...
SNR_weights = 15:5:40;   % New variable for different SNR weights
window 		= 6;
...
MSE_dB_LMMSE	=zeros(length(SNR_weights), length(SNR_in_dB)); % Modify to 2D array

for i=1:length(SNR_weights) % New loop for different SNR weights
    SNR_weight = SNR_weights(i);
    ...
    for a=1:length(SNR_in_dB)
        ...
        MSE_dB_LMMSE(i, a)	= 10*log10( MSE_LMMSE / (1644*40*frame_num) ); % Change to 2D array
    end
end
...
hold on;
color = ['g', 'b', 'c', 'm', 'y', 'k']; % Colors for different SNR weights
for i=1:length(SNR_weights)
