...
%output
MSE_dB_LS		= zeros(length(Windows), length(SNR_in_dB));
MSE_dB_LMMSE	= zeros(length(Windows), length(SNR_in_dB));

for w = 1:length(Windows)
    %LMMSE 權重矩陣
    W = make_W_S(Windows(w),SNR_weight);
    ...
    for a=1:length(SNR_in_dB)
        ...
        parfor frame=1:frame_num
            fprintf("Window: %d/%d \t SNR : %d/%d \t frame : %d/%d\n", w, length(Windows), a,length(SNR_in_dB),frame,frame_num);
